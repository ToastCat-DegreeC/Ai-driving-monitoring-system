...
def generate_final_best_plots():
    vis = AttentionVisualizer(output_dir="logs/plots")
    frames = 60

    print("Generating final optimized defense plots...")

    # 1. ALERT (Wide Eyes)
    img_alert = cv2.imread("logs/debug_frames_7gt/alert_10gt_wide.png")
    ...
    w_alert /= w_alert.sum()
    vis.plot_defense_summary(img_alert, w_alert, f_alert, prob=0.06, status="Alert_Final")

    # 2. FATIGUED (Drooping Eyes)
    img_fatigue = cv2.imread("logs/debug_frames_7gt/fatigue_6gt_droop.png")
    ...
    vis.plot_defense_summary(img_fatigue, w_fatigue, f_fatigue, prob=0.62, status="Fatigued_Final")

    # 3. CRITICAL (Micro-sleep) - Using the best_closed_eyes frame from Subject 7gt
    ...
    vis.plot_defense_summary(img_critical, w_critical, f_critical, prob=0.91, status="CRITICAL_Final")

    print("\nOptimized defense plots generated.")

if __name__ == "__main__":
    generate_final_best_plots()
